...
export default function GovernancePage() {
  ...
  const fetchManuals = async (currentFilters = filters) => {
    setManualsLoading(true);
    setManualsError(null);
    try {
      const result = await governanceService.getManuals({
        systemCode: currentFilters.systemCode || undefined,
        status: currentFilters.status || undefined,
        risk: currentFilters.risk || undefined,
        priority: currentFilters.priority || undefined,
        responsible: currentFilters.responsible || undefined,
        issueType: currentFilters.issueType || undefined,
      });
  ...
  const riskOptions = useMemo(() => uniqueOptions(manuals.map((manual) => manual?.risk)), [manuals]);
  const priorityOptions = useMemo(() => uniqueOptions(manuals.map((manual) => manual?.priority)), [manuals]);
  const responsibleOptions = useMemo(() => uniqueOptions(manuals.map((manual) => manual?.responsible)), [manuals]);
  const issueTypeOptions = useMemo(
    () => uniqueOptions(manuals.flatMap((manual) => manual?.issueTypes || [])),
    [manuals]
  );
  ...
  const handleAction = async (manualId: string, action: string, handler: () => Promise<void>) => {
    ...
    try {
      ...
      await Promise.all([fetchSummary(), fetchManuals()]);
    } catch (err) {
    ...
    <MainLayout>
      ...
      <div className="card-metric mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => setFilters((prev) => ({ ...prev, systemCode: value === 'ALL' ? '' : value }))}
            >
              <SelectTrigger><SelectValue placeholder="Todos os sistemas" /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => setFilters((prev) => ({ ...prev, risk: value === 'ALL' ? '' : value }))}
            >
              <SelectTrigger><SelectValue placeholder="Todos os riscos" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="ALL">Todos</SelectItem>
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => setFilters((prev) => ({ ...prev, responsible: value === 'ALL' ? '' : value }))}
            >
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger><SelectValue placeholder="Todos os problemas" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="ALL">Todos</SelectItem>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <Button variant="secondary" onClick={() => fetchManuals()}>
      ...
      {summaryLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[1, 2, 3, 4].map((item) => <LoadingSkeleton key={item} variant="card" />)}
        </div>
      ) : summaryError ? (
        <div className="card-metric mb-6">
              ...
              <tbody>
                {manuals.map((manual, index) => {
                  ...
                  const isActionLoading = (action: string) =>
                    actionLoading?.id === manualId && actionLoading?.action === action;
                      ...
                      <td className="p-4">
                        <div className="flex flex-wrap gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => setAssignTarget(manual)}
                          >
