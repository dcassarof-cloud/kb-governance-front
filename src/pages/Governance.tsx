...
export default function GovernancePage() {
  ...
  return (
    <MainLayout>
      ...
      {!manualsLoading && !manualsError && (manualsData?.totalPages ?? 0) > 1 && (
        <div className="flex items-center justify-between mt-4">
          <p className="text-sm text-muted-foreground">
            Página {page} de {manualsData?.totalPages ?? 1}
          </p>
          <div className="flex gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setPage((prev) => Math.min(manualsData?.totalPages ?? prev, prev + 1))}
              disabled={page >= (manualsData?.totalPages ?? 1)}
            >
              Próxima
            </Button>
          </div>
